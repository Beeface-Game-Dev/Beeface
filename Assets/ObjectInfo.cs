...
public class ObjectInfo : MonoBehaviour {
    ...
    void Update () {
        ...
        {
            Vector3 spawn = new Vector3(200, -48, 180);
            Colonist.GetComponent<NavMeshAgent>().Warp(spawn);

           if (clone = GameObject.Find("Colonist(Clone)"))
           {
                agent = GetComponent<NavMeshAgent>();
                agent.enabled = false;
                Invoke("EnableNavMeshAgent", 0.1f);
                agent.enabled = true;
                Debug.Log("agent true");
           }
        }
        ...
        if (isColonist)
        {
            if (task == TaskList.Gathering) //Is the Colonist currently gathering?
            {
                ...
                if (distToTarget <= 30f) //Is the distance to the target less than 3.5?
                {
                    Gather(); //Calls the Gather() Method
                    // Debug.Log("At gather pos");
                }
            }
            ...
            if (task == TaskList.Delivering) //Is the Colonist currently delivering?
            {
                ...
                //Debug.Log("task " + task);
                if (distToTarget <= 25f) //Is the distance to the target drop less than 4?
                {
                    //Debug.Log("dist");
                   // Debug.Log("maxf "+RM.maxFood);
            ...
            if (heldResource >= maxHeldResource && isGathering == true)//Is the colonist carrying the max amount of resources?
            {
                if (targetNode.GetComponent<ObjectInfo>().gatherers > 0)
                {
                    targetNode.GetComponent<ObjectInfo>().gatherers--; //Remove itself from the node's gatherers
                    Debug.Log("gatherers " + gatherers);
                }
        ...
        if (task == TaskList.Attacking)
        {
            if (target)
            {
                ...
                {
                        canAttack = false;
                        if (distance - dist >= 100 || distance - dist <= -100)
                        {
                            //Debug.Log("dist " + distance);
                            //Debug.Log("modD " + distance+1);
                            distance = dist;
                ...
                else if (dist <= range)//Atack distance = 40 anders kani atak
                {
                    canAttack = true;
                    Debug.Log("in range");
                }
        ...
        //Debug.Log("iconcam "+ iconCam);
    }

    /*IEnumerator atackpause()
    {
        bool pause;
        pause = true;
        while (pause == true) //Is this CoRoutine running?
        {
            yield return new WaitForSeconds(1f); //Wait for seconds equal to this object's attack speed
            //agent.destination = target.transform.position;
            Debug.Log("atackpause");
            pause = false;
        }
    }*/
    ...
    {
        if (gatherers != 0 && gatherers >0) //Are there any units gathering from this node?
        {
            availableResource = availableResource - gatherers; //Subtract the current number of units gathering from the available resources
            Debug.Log("gatherers na " + gatherers);
        }
    }
    ...
    GameObject GetClosestDropOff(GameObject[] dropOffs)
    {
        ...
        Debug.Log("getdropoff "+ closestDrop);
    }
    ...
    public void RightClick()
    {
        ...
        if (Physics.Raycast(ray, out hit, 1000)) //Did the ray hit anything?
        {
            ...
            else if (hit.collider.tag == "Drops" && isSelected == isPlayerObject) //Did the player click a drop point?
            {
                if (targetNode.GetComponent<ObjectInfo>().gatherers > 0)
                {
                    targetNode.GetComponent<ObjectInfo>().gatherers--; //Remove the colonist from the node's gatherers
            ...
            else if(hit.collider.tag == "Selectable"|| hit.collider.tag == "enemywirm" || (hit.collider.tag == "Drops" && isSelected!= isPlayerObject)) //Did the player click a selectable object?
            {
    ...
    public void Gather()
    {
       ...
       // Debug.Log("isgather "+ isGathering);
        if (!isGatherer) //Is the colonist a gatherer?
        {
           ...
           // Debug.Log("isgatherer "+ isGatherer);
        }
    ...
    private void OnMouseOver()
    {
        ...
        if(gameObject.name.Equals("Colonist(Clone)"))
        {
            toolTipInfo = "Worker Beeface is used to gather resourses but is weak in atack";
            toolTipInfo = Colonist.gameObject.GetComponent<ObjectInfo>().toolTipInfo;
            GM.tooltipInfo = toolTipInfo;
        }
        else
        {
            //Debug.Log("info " + toolTipInfo);
            GM.tooltipInfo = toolTipInfo;
            //Debug.Log("info " + toolTipInfo);
        }
          //Thread.Sleep(300);
